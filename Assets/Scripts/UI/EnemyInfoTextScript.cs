...
namespace TowerDefense
{
    ...
    public class EnemyInfoTextScript : MonoBehaviour
    {
        ...
        void Start()
        {
            _myCanvas = GameObject.Find("HUDCanvas").GetComponent<Canvas>();//GetComponentInParent<Canvas>().GetComponent<RectTransform>();
        ...
        void LateUpdate() {
            if (objectToFollow && !objectToFollow.gameObject.activeInHierarchy) {
                ...
                transform.position = WorldToUISpace(_myCanvas, objectToFollow.transform.position) - screenOffset;
            }
        }
